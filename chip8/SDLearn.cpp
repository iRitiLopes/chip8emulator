...
SDL_AppResult SDL_AppIterate(void* appstate)
{
    ...
    if (SDL_LockTextureToSurface(texture, NULL, &surface)) {
        ...
        r[0].w = TEXTURE_SIZE;
        r[0].h = TEXTURE_SIZE / 10;
        r[0].x = 0;
        r[0].y = (int)(((float)(TEXTURE_SIZE - r[0].h)) * ((scale + 1.0f) / 2.0f));

        r[1].w = TEXTURE_SIZE;
        r[1].h = TEXTURE_SIZE / 15;
        r[1].x = 0;
        r[1].y = (int)(((float)(TEXTURE_SIZE - r[1].h)) * ((scale + 1.0f) / 2.0f)) / 10;
        ...
        SDL_FillSurfaceRects(surface, r, 2, SDL_MapRGB(SDL_GetPixelFormatDetails(surface->format), NULL, 0, 125, 125));
